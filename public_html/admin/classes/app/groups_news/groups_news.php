...
class groups_news 
{
    ...
    protected function form()
    {
        ...
        $records=mysql_db::exec_query("select groups.name as `group`,groups.id as group_id,groups_news.news as news
                                                FROM groups 
                                                left outer join groups_news ON groups.id=groups_news.groups and groups_news.news='".$this->get[id]."'
                                                left outer join content ON content.id=groups_news.news and content.id='".$this->get[id]."' 
        ...
        {
            if(!empty($record['news'])){$checked=" checked";}else{$checked="";}
    ...
    protected function act()
    {
        //print($this->get[page]);
        //print_r($this->post);
        mysql_db::exec_query("delete from ".$this->get['page']." where news=".$this->get['id']);
        foreach($this->post[group] as $key=>$value)
        {
            //print($key.":".$value."<br />");
            mysql_db::exec_query("insert into ".$this->get['page']." set news='".$this->get['id']."',groups='".$key."'");
        }
        box::showSuccesMessage('main.php','&page=content&class=content','شكراً');
