...
class news_type 
{
    ...
    {
        print('<h1 align="right">تحرير أنواع الخبر</h1>');
        ...
        form::close_form();
        print('</tr>');
        print('<tr bgcolor="#a0a0a0"><td>الترتيب</td><td>الاسم</td><td>الحالة</td><td>تعديل</td><td>حذف</td></tr>');
        while($res=mysql_fetch_array($re))
        {
            print('<tr bgcolor="#f0f0f0"><td>'.$res['ordered']);
            print(' &nbsp; <a href="'.box::href_req("page=".$this->get['page']."&class=".$this->get['class']."&event=sort&no=".($res['ordered'])."&state=+1&id=".$res['id']).'"><img src="images/sort1.png" border="0" /></a>');
            if($res['ordered']>=2)
            {
                print(' &nbsp; <a href="'.box::href_req("page=".$this->get['page']."&class=".$this->get['class']."&event=sort&no=".($res['ordered'])."&state=-1&id=".$res['id']).'"><img src="images/sort0.png" border="0" />');
            }
            print('</td><td>'.$res['name'].'</td><td>'.$this->status[$res['enabled']].'</td></td><td><a href="'.box::popup("page=".$this->get['page']."&class=".$this->get['class']."&event=form&id=$res[id]").'">تعديل</a></td><td><a href="javascript:if(confirm(\'هل انت متأكد\')){'.box::popup("page=".$this->get['page']."&class=".$this->get['class']."&event=delete&id=$res[id]").'}">حذف</a></td></tr>');
        }
