...
class groups_members 
{
    ...
    {
        print('<h1 align="right">تحرير مجموعات الأعضاء</h1>');
    ...
    {
        print('<div style="border-style:solid;border-color:#c6;border-width: 1px;">');
        //$record=mysql_db::get_records_by_key("select * from ".$this->get[page]." where members=".$this->get[id]."");
        $records=mysql_db::exec_query("select groups.name as `group`,groups.id as group_id,members.id as member_id 
                                                FROM groups 
                                                left outer join groups_members ON groups.id=groups_members.groups and groups_members.members='".$this->get[id]."'
                                                left outer join members ON members.id=groups_members.members and members.id='".$this->get[id]."' 
        ...
        print('<tr><td></td><td>المجموعة</td></tr>');
        while($record=mysql_fetch_array($records))
        {
            if(!empty($record['member_id'])){$checked=" checked";}else{$checked="";}
    ...
    protected function act()
    {
        //print($this->get[page]);
        //print_r($this->post);
        mysql_db::exec_query("delete from ".$this->get['page']." where members=".$this->get['id']);
        foreach($this->post[group] as $key=>$value)
        {
            //print($key.":".$value."<br />");
            mysql_db::exec_query("insert into ".$this->get['page']." set members='".$this->get['id']."',groups='".$key."'");
        }
        box::showSuccesMessage('main.php','&page=members&class=members','شكراً');
