...
class content 
{
    ...
    protected $case="";
    protected $post="";
    protected $status = Array(1=>_DEACTIVATE,2=>_ACTIVATE);
    protected $appearance = Array(1=>'تعطيل',2=>'أسلوب عرض سجلات',3=>'Featured Content Slider by: Chris Coyier',4=>'Featured Content Slider',5=>'jcontent:horizontal,left',6=>'jcontent:horizontal,right',7=>'jcontent:vertical,top',8=>'jcontent:vertical,down',9=>'jcontent:horizontal,button',10=>'jcontent:vertical,button',11=>'قلاب شريط شفاف مع صورة كبيرة',12=>'قلاب مع صورة',13=>'عمودين',14=>'صناديق',16=>'Accordion',17=>'Accordion full image',18=>'once content beside',19=>'two content beside',23=>'عمودين من العناوين',24=>'عمودين من العناوين مع التاريخ والمصدر',30=>'icons slider',31=>'slider kit','33'=>'عمودين من عناوين');
    protected $appearance_selections= Array(1=>'تعطيل',2=>'أسلوب عرض سجلات',3=>'Featured Content Slider by: Chris Coyier',4=>'Featured Content Slider',5=>'jcontent:horizontal,left',6=>'jcontent:horizontal,right',7=>'jcontent:vertical,top',8=>'jcontent:vertical,down',9=>'jcontent:horizontal,button',10=>'jcontent:vertical,button',11=>'شريط شفاف مع صورة كبيرة',12=>'قلاب مع صورة',13=>'عمودين',14=>'صناديق',15=>'حاويات',16=>'Accordion',17=>'Accordion full image',19=>'two content beside',20=>'a box contains multi columns',21=>'two boxes columns with the first content details',22=>'two boxes columns with the first tow content details',25=>'عمودين من العناوين مع التاريخ والمصدر',26=>'multi boxes contain titles',27=>'multi boxes contain titles with date and source',30=>'icons slider',31=>'slider kit',32=>'tabs','33'=>'عمودين من عناوين'); 
    ...
    protected $type= Array('sec'=>'صفحة متفرعة','body'=>'صفحة محتوى','ext'=>'تطيبق ويب','news'=>'إخبارية','files'=>'ملفات','pro'=>'منتجات','vedio'=>'فيديو','image'=>'صور','sound'=>'صوت','link'=>'ارتباط','books'=>'كتب','selections'=>'مختارات','sections'=>'أقسام','once_article'=>'محتوى واحد من كل قسم');
...
//    }
    ...
    protected function browse()
    {
        ...
        {
            if($res['type']=='vedio'){$status=$this->appearance_video;}elseif($res['type']=='image'){$status=$this->appearance_photos;}elseif($res['type']=='news' || $res['type']=='books' || $res['type']=='selections'|| $res['type']=='once_article' || $res['type']=='sections'){$status=$this->appearance_selections;}else{$status=$this->status;}
    ...
    protected function act()
    {
        ...
        if(empty($this->post['_date']) && $class=='books')
        {
            box::showSuccesMessage('main.php','&page='.$this->params[page].'&class='.$class.'&id='.$this->post['parent'].'&div=body','<br />عذراً لم تملأ حقل التاريخ..........<br />','error.png','body');
        }
        elseif(empty($this->post['title']) || empty($this->post['language']) || empty($this->post['descr']) || empty($this->post['keywords']))
        {
    ...
    protected function form()
    {
        ...
        if($record['parent'][0]!='')
        {
            ...
            else//ended an article
            {
                ...
                print('<tr><td>التاريخ:</td><td>');form::add_input('_date','text','<font color="#ff0000">*</font>',$record[_date][0],'id="_date" dir="ltr"');print('<script type="text/javascript">Calendar.setup({inputField:"_date",button:"_date",align:"Br"});</script>');print('</td></tr>');
