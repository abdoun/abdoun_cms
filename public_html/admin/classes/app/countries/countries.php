...
class countries 
{
    ...
    {
        print('<h1 align="right">تحرير الدول</h1>');
        ...
        if($this->post['code']!=''){$sql.=" and code like '%".$this->post['code']."%' ";}
        ...
        print('<td></td><td>');
        form::add_input('code','text','',$this->post['code'],' title="الرمز"');
        print('</td><td>');
        form::add_input('name','text','',$this->post['name'],' title="الدولة"');
        print('</td><td>');
        ...
        form::close_form();
        print('</tr>');
        print('<tr bgcolor="#a0a0a0"><td></td><td>الرمز</td><td>الدوية</td><td>الحالة</td><td>تعديل</td><td>حذف</td></tr>');
        while($res=mysql_fetch_array($re))
        {
            print('<tr bgcolor="#f0f0f0"><td>'.$res['id'].'</td><td>'.$res['code'].'</td><td>'.$res['name'].'</td><td>'.$this->status[$res['enabled']].'</td></td><td><a href="'.box::popup("page=".$this->get['page']."&class=".$this->get['class']."&event=form&id=$res[id]").'">تعديل</a></td><td><a href="javascript:if(confirm(\'هل انت متأكد\')){'.box::popup("page=".$this->get['page']."&class=".$this->get['class']."&event=delete&id=$res[id]").'}">حذف</a></td></tr>');
        }
        ...
        form::add_input('code','hidden','',$this->post['code']);
    ...
    protected function form()
    {
        ...
        print('<tr bgcolor="#cccccc"><td>الرمز:</td><td>');form::add_input('code','text','',$record[code][0]);print('</td></tr>');
        print('<tr bgcolor="#cccccc"><td>الدولة:</td><td>');form::add_input('name','text','',$record[name][0]);print('</td></tr>');
