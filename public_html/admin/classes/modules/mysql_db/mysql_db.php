...
class mysql_db
{
    	   ...
    	   while($res=mysql_fetch_array($re))
    		{
    		  //$i=0;
    			...
    			{
    			     //print(mysql_field_type($re,$i).':'.mysql_field_name($re,$i).'<br />');
    			     if($key!='body')
                     {
    ...
    public function maxx($table,$field,$query='')
    {
        ...
        $query=mysql_real_escape_string($query);
         $no=mysql_fetch_row(mysql_query("select max($field) from $table $query"));//print("select max($field) from $table $query");
         return $no[0]+1;
    }
    ...
    public function add_edit_rec($table,$arr,$id='',$query='')
    {
        ...
        if(!empty($table))
        {
            ...
            {
                print("$sql $table $where $query<br />".mysql_error());
