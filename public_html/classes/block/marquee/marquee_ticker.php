...
class marquee 
{
    ...
    protected function data_menu($l,& $content='')
    {
       ...
       for($i=0;$i<count($res['id']);$i++)
       {
            ...
            {
                $content.='<div class="sliderkit-panel"><a class="marquee" title="'.$res['title'][$i].' '.$res['descr'][$i].'"'.$selected.' href="'.$href.'?l='.$l.'&id='.$res['id'][$i].'&type='.$res['type'][$i].'&page=content&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'">';
                $content.=$res['title'][$i];
                $content.='</a></div>';
            ...
            {
                $content.='<div class="sliderkit-panel"><a title="'.$res['title'][$i].' '.$res['descr'][$i].'"'.$selected.' href="'.$href.'?l='.$l.'&id='.$res['id'][$i].'&type='.$res['type'][$i].'&page=content&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'">';
                $content.=$res['title'][$i];
                $content.='</a></div>';
                //$content.=' <img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/icon.png" align="center" height="20" /> ';
            }
    ...
    function view($l='')
    {
       ...
       $html=str_replace('<img_tag_ />','<img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/icon.png" height="20" />',$html);//
