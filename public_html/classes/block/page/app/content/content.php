...
class content
{
    ...
    protected function get_title($id='')
    {
        ...
        {
            $re=mysql_db::get_records_by_key("select title from content where id=$id");
            return $re[title][0];
        }        
    }
...
//        $caption = ob_get_contents();
//        ob_end_clean();
        if($this->params['main']===true){$caption='';}
    ...
    protected function browse()
    {
        ...
        block_::box_border('<span>'.$resu['descr'][0].'</span>');
        print('<table width="100%"><tr>');
        while($res=mysql_fetch_array($re))
        {
            if($res['permission']==1 || ($res['permission']==2 && $_SESSION['username']!='' && membership::content_member($res['id'])))
            {
                $img=getimagesize('http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$res['icon']);
                //print_r($img);
                if($img[0]>200){$img[0]='200';}
...
class image extends content
{
    protected function browse()
    {
        ...
        {
            @$files=dir($re['body'][0]);
            block_::box_border('<span>'.$re['descr'][0].' '.$share.'</span>');
            ...
            if($res['permission']==1 || ($res['permission']==2 && $_SESSION['username']!='' && membership::content_member($res['id'])))
            {
                ...
                if(substr($res['body'],-2)=='rm' || substr($res['body'],-3)=='ram')
                {
                    ...
                    <object classid="clsid:CFCDAA03-8BE4-11cf-B84B-0020AFBBCCFA" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0" width="240" height="30">
                    	...
                    	<spanbed type="audio/x-pn-realaudio-plugin" src="<?=$res['body'];?>" width="240" height="30"></embed>
                    </object></td><?
...
class vedio extends content
{
    protected function browse()
    {
        ...
        {
            $_GET['dir']='';//vedio
            require(''.$_SERVER["DOCUMENT_ROOT"].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'js/featured_contents/jcontent_examples/jcontent_vedio.php');
        }
        else
        {
    ...
    protected function view()
    {
        ...
        if(!empty($res['body'][0]))
        {
            ...
            if($ext=='avi' || $ext=='wmv')
            {
                ...
                <object classid="clsid:6BF52A52-394A-11D3-B153-00C04F79FAA6" codebase="http://activex.microsoft.com/activex/controls/mplayer/en/nsmp2inf.cab#Version=5,1,52,701" width="560" height="315">
                	...
                	<spanbed type="application/x-mplayer2" src="<?=$res['body'][0];?>" width="560" height="315"></embed>
                </object>    
                <?
            }
            elseif($ext=='mp4')
            {
                ...
                <object classid="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B" codebase="http://www.apple.com/qtactivex/qtplugin.cab#version=6,0,2,0" width="560" height="315">
                	...
                	<param name="width" value="560" />
                	<param name="height" value="315" />
                	<spanbed type="vedio/quicktime" autoplay="true" src="<?=$res['body'][0];?>" width="400" height="315"></embed>
                </object>
                <?
            }
            elseif($ext=='flv')
        ...
        <?  }
            elseif($ext=='ram' || substr($res['body'][0],-2)=='rm' || $ext=='mp3')
            {
            	...
            	<param name="width" value="560" />
            	<param name="height" value="300" />
            	<spanbed type="audio/x-pn-realaudio-plugin" src="<?=$res['body'][0];?>" width="400" height="300"></embed>
            </object><?
            }
            else
            {
...
class selections extends books
{
    protected function browse()
    {
        ...
        ob_start();
        //print('<br />');
        if($this->params['main']===true){$limit=4;}else{$limit=8;}
...
class once_article extends selections
{
    protected function browse()
    {
        $this->set_meta_desc();
        ...
        ob_start();
        //print('<br />');
        if($this->params['main']===true){$limit=1;}else{$limit=1;}
        //if(!empty($this->params['q_sec'])){$sql=" and (a.title like '%".$this->params['q_sec']."%' or a.descr like '%".$this->params['q_sec']."%' or a.body like '%".$this->params['q_sec']."%' or a.keywords like '%".$this->params['q_sec']."%' or c.name like '%".$this->params['q_sec']."%' or b.name like '%".$this->params['q_sec']."%')";}
        $resu=mysql_db::get_records_by_key("select id,permission,descr,enabled,main,body from content where id='".$this->params['id']."' and enabled>1");
        $ids=explode(',',$resu['body'][0]);
        $no=count($ids);
...
{
    protected function share($res,$id)
    {
        if($res['type'][$id]=='selections')
        {
            return '';
        }
        else
        {
            return '<!-- AddThis Button BEGIN -->
                <div class="addthis_toolbox addthis_default_style "
                addthis:url="?l='.$this->params['l'].'&id='.$res['id'][$id].'&type='.$res['type'][$id].'&page='.$this->params['page'].'&event=view&title='.clean::clean_url($res[title][$id]).'"
                addthis:title="'.$res['title'][$id].'"
                addthis:description="'.$res['descr'][$id].'">
        ...
        switch($fc)
        {
            ...
            $u=1;
                for($i=0;$i<count($res['id']);$i++)
                {
                    if($res['permission'][$i]==1 || ($res['permission'][$i]==2 && $_SESSION['username']!='' && membership::content_member($res['id'][$i])))
                    {
                        if($res['type'][$i]!='' && $res['type'][$i]!='news' && $res['type'][$i]!='books' && $res['type'][$i]!='vedio' && $res['type'][$i]!='files' && $res['type'][$i]!='pro')
                        {
                        ...
                        else
                        {
                            ...
                            {
                                $res['type'][$i]=$this->params['type'];
                                $event='view';
                            }
                        }
                        if(!empty($res['_file'][$i])){if(strpos($res['_file'][$i],'.')===false){$_file=' <span style="color:#969696;">'.$res[_file][$i].'</span> ';}else{$_file='';}}else{$_file='';}
                        ...
                        print('<td dir="'._DIR.'" style="font-family: tahoma;font-size:13px;vertical-align:top;" width="400" valign="top">'.$img.'<b>'.$res[title][$i].'</b><br /><br /><span style="color:#808080">'.$res[_author][$i].'</span>'.$_file.'<br /><br /><span style="color:#808080;">'.$res[_date][$i].'</span><br /><br /><span style="font-size:12px;">'.$res['descr'][$i].'</span> <div class="more"><a  href="?l='.$res['language'][$i].'&id='.$res['id'][$i].'&type='.$resu['type'][0].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'" style="color:#8E0707;font-size:14px;">'._MORE.'...</a><br /><br />'.$this->share($res,$i).'<br /><hr style="background-color: #ccc;height: 1px;" height="1" color="#cccccc" /></td><td>&nbsp;</td>');
            ...
            $u=1;
                for($i=0;$i<count($res['id']);$i++)
                {
                    if($res['permission'][$i]==1 || ($res['permission'][$i]==2 && $_SESSION['username']!='' && membership::content_member($res['id'][$i])))
                    {
                        ...
                        if(!empty($res['icon'][$i])){$img='<img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$res['icon'][$i].'" width="120" align="'._ALIGN_.'" />';}
                        print('<td dir="'._DIR.'" align="'._ALIGN_.'" style="font-family: Times New Roman,Arial, Helvetica, Sans-Serif;font-size:11px;vertical-align:top;text-align:center" width="390" valign="top">');
                        echo str_replace('<tag />',$img.'<span style="color:#969696;border:#333 solid 1px;">'.$res[_author][$i].'</span>'.$_file.'<br /><span style="color:#969696;border:#333 solid 1px;">'.$res[_date][$i].'</span><br /><br />'.$res['descr'][$i].' <div class="more"><a  href="?l='.$res['language'][$i].'&id='.$res['id'][$i].'&type='.$resu['type'][0].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'" style="color:#8E0707;font-size:11px;">'._MORE.'...</a><br /><br />'.$this->share($res,$i),str_replace('<title_ />','<b>'.$res[title][$i].'</b>',$box));
                        print('</td><td>&nbsp;</td>');
                        if($u%2==0){print('</tr><tr>');}
                        $u++;
                    }
            ...
            if(!$this->params['main']){print('<span>'.$resu[descr][0].'</span>');}
            $box=$this->template_include('boxes.html');
            print('<table width="100%" border="0"><tr>');
            $u=1;
            ...
            foreach($cont as $k=>$v)
            {
                $item='<ul style="margin:0;padding:0;width:100%">';
                print('<td dir="'._DIR.'" valign="top" width="400">');
                foreach($v as $kk=>$vv)
                {
                    if($vv['type']!='' && $vv['type']!='news' && $vv['type']!='books' && $vv['type']!='vedio' && $vv['type']!='files' && $vv['type']!='pro')
                    {
                    ...
                    $item.='<li><a title="'.clean::burn($vv[descr]).'" href="?l='.$vv['language'].'&id='.$vv['id'].'&type='.$vv['type'].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($vv['title']).'">'.clean::burn($vv[title]).'</a></li>';
                }
                $item.='</ul>';
                echo str_replace('<title_ />','<a  href="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'rss.php?l='.$this->params['l'].'&id='.$k.'"><img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'images/rssicon.png" height="20" title="rss" alt="RSS" border="0" align="'._ALIGN_.'" style="padding: 5px" /></a><b>'.$this->get_title($k).'</b>',str_replace('<tag />',$item,$box));
                print('</td>');
                if($u%2==0){print('</tr><tr>');}
                $u++;
            }
            ...
            for($i=0;$i<count($res['id']);$i++)
            {
                if($res['permission'][$i]==1 || ($res['permission'][$i]==2 && $_SESSION['username']!='' && membership::content_member($res['id'][$i])))
                {
                    ...
                    if(!empty($res['_file'][$i])){if(strpos($res['_file'][$i],'.')===false){$_file=' <span style="background-color:#ccc">'.$res[_file][$i].'</span> ';}else{$_file='';}}else{$_file='';}
                    if(!empty($res['icon'][$i])){$img='<img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$res['icon'][$i].'" width="120" height="150" align="'._ALIGN.'" style="padding:5px" />';}
                    if($u==1)
                    {
                        $content='<div style="float:'._ALIGN.';height:150px;width:320px;overflow: hidden;border:0px solid #ccc;border-left-width:1px;text-align:justify;padding:5px;">'
                        .$img.
                        '<div style="font-weight: bold;"><a  href="?l='.$res['language'][$i].'&id='.$res['id'][$i].'&type='.$resu['type'][0].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'">'
                        .$res[title][$i].
                        '</a></div><div style="color:#969696">'
                        .$res[_author][$i].
                        '</div><div style="color:#969696">'
                        .$res[_date][$i].
                        '</div>'
                        .$res['descr'][$i].
                        '</div>';
                    ...
                    {
                        $menu.='<li><a style="font-size: 14px;font-family: traditional arabic,Times New Roman;font-weight: bold;" href="?l='.$res['language'][$i].'&id='.$res['id'][$i].'&type='.$resu['type'][0].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'">'.$res[title][$i].'</a></li>';
                    }
                    //print('<td dir="'._DIR.'" align="'._ALIGN_.'" style="font-family: Times New Roman,Arial, Helvetica, Sans-Serif;font-size:11px;vertical-align:top;text-align:center" width="390" valign="top">');
                    //echo str_replace('<tag />',$img.'<span style="color:#969696;border:#333 solid 1px;">'.$res[_author][$i].'</span>'.$_file.'<br /><span style="color:#969696;border:#333 solid 1px;">'.$res[_date][$i].'</span><br /><br />'.$res['descr'][$i].' <div class="more"><a  href="?l='.$res['language'][$i].'&id='.$res['id'][$i].'&type='.$resu['type'][0].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'" style="color:#8E0707;font-size:11px;">'._MORE.'...</a><br /><br />'.$this->share($res,$i),str_replace('<title_ />','<b>'.$res[title][$i].'</b>',$box));
                    $u++;
            ...
            block_::box_border($content.'<div style="float:'._ALIGN_.';height:150px;width:340px;overflow: auto;"><ul style="padding:0;margin:0">'.$menu.'</ul></div>');
            break;
            case "19"://two content beside
            if(!$this->params['main']){print('<span>'.$resu[descr][0].'</span>');}
            //$box=$this->template_include('boxes.html');
            $u=1;
            $content='<div style="float:'._ALIGN.';height:180px;width:320px;overflow: hidden;border:0px solid #ccc;border-left-width:1px;text-align:justify;padding:5px;">';
            for($i=0;$i<count($res['id']);$i++)
            {
                if($res['permission'][$i]==1 || ($res['permission'][$i]==2 && $_SESSION['username']!='' && membership::content_member($res['id'][$i])))
                {
                    ...
                    if(!empty($res['_file'][$i])){if(strpos($res['_file'][$i],'.')===false){$_file=' <span style="background-color:#ccc">'.$res[_file][$i].'</span> ';}else{$_file='';}}else{$_file='';}
                    if(!empty($res['icon'][$i])){$img='<img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$res['icon'][$i].'" width="60" height="75" align="'._ALIGN.'" style="padding:5px" />';}
                    
                    if($u<=2)
                    {
                        $content.='<div style="height:80px;overflow: hidden;" title="'.$res['descr'][$i].'">';
                        $content.=$img;
                        $content.='<div style="font-weight: bold;"><a style="font-size:14px" href="?l='.$res['language'][$i].'&id='.$res['id'][$i].'&type='.$resu['type'][0].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'">';
                        $content.=$res[title][$i];
                        $content.='</a></div>';
                        $content.='<div style="color:#969696;">';
                        $content.=$res[_author][$i];
                        $content.='</div><div style="color:#969696">';
                        $content.=$res[_date][$i];
                        //$content.='</div>';
                        //$content.='<div style="color:#969696">';
                        //$content.=$res['descr'][$i];
            ...
            $content.='</div>';
            block_::box_border($content.'<div style="float:'._ALIGN_.';height:180px;width:340px;overflow: auto;"><ul style="padding:0;margin:0">'.$menu.'</ul></div>');
            break;
            case "20"://a box contains multi columns
            if(!$this->params['main']){print('<span>'.$resu[descr][0].'</span>');}
            //$box=$this->template_include('boxes.html');
            ...
            foreach($cont as $k=>$v)
            {
                //print('<div style="text-direction:ltr;direction:ltr" dir="ltr">'.print_r($v).'</div>');
                if($u!=1){$content.='<td style="border: 0px solid #ccc;border-'._ALIGN.'-width:1px;"> <td>';}
                $content.='<td dir="'._DIR.'" valign="top"><a  href="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'rss.php?l='.$this->params['l'].'&id='.$k.'"><img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'images/rssicon.png" height="20" title="rss" alt="RSS" border="0" align="'._ALIGN_.'" style="padding: 5px" /></a><b>'.$this->get_title($k).'</b>';
                $content.='<ul style="margin:0;padding:0;margin-top:15px;width:100%;">';
                foreach($v as $kk=>$vv)
                {
                    if($vv['type']!='' && $vv['type']!='news' && $vv['type']!='books' && $vv['type']!='vedio' && $vv['type']!='files' && $vv['type']!='pro')
                    {
                    ...
                    $content.='<li><a title="'.clean::burn($vv[descr]).'" href="?l='.$vv['language'].'&id='.$vv['id'].'&type='.$vv['type'].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($vv['title']).'">'.clean::burn($vv[title]).'</a><br /><span style="color:#969696;float:right">'.$vv['_author'].'</span><span style="color:#969696;float:left">'.$vv['_date'].'</span><div style="clear:both"></div></li>';
                }
                $content.='</ul>';
                //echo str_replace('<title_ />','<a  href="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'rss.php?l='.$vv['language'].'&id='.$vv[id].'"><img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'images/rssicon.png" height="20" title="rss" alt="RSS" border="0" align="'._ALIGN_.'" style="padding: 5px" /></a><b>'.$v[$kk][title_parent].'</b>',str_replace('<tag />',$item,$box));
                $content.='</td>';
                //if($u%2==0){print('</tr><tr>');}
                $u++;
            }
            $content.='</tr></table>';
            block_::box_border($content);
            if(!$this->params['main'])
            {
                for($j=0;$j<$no;$j+=20)
                {
            ...
            break;
            case "21"://two boxes columns with the first content details
            if(!$this->params['main']){print('<span>'.$resu[descr][0].'</span>');}
            $box=$this->template_include('boxes.html');
            print('<table width="100%" border="0"><tr>');
            $u=1;
            ...
            {
                $item='<ul style="margin:0;padding:0;width:100%">';
                print('<td dir="'._DIR.'" valign="top" width="45%">');
                $j=1;
                foreach($v as $kk=>$vv)
                {
                    if($vv['type']!='' && $vv['type']!='news' && $vv['type']!='books' && $vv['type']!='vedio' && $vv['type']!='files' && $vv['type']!='pro')
                    ...
                    if(!empty($vv['icon'])){$img='<img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$vv['icon'].'" height="100" style="padding:5px" />';}
                    if($j==1)
                    {
                        $item.='<li style="list-style-type: none;border:0 #ccc;border-bottom-width:1px;border-style:solid;margin-bottom:15px"><p align="center">'.$img.'</p>'
                        .'<div style="font-weight: bold;"><a style="font-size:12px" title="'.clean::burn($vv[descr]).'" href="?l='.$vv['language'].'&id='.$vv['id'].'&type='.$vv['type'].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($vv['title']).'">'.clean::burn($vv[title]).'</a>'
                        .'</div><span style="color:#969696;float:right">'.$vv['_author'].'</span><span style="color:#969696;float:left">'
                        .$vv['_date']
                        .'</span><br />'
                        .$vv['descr']                        
            ...
            break;
            case "22"://two boxes columns with the first two content details
            if(!$this->params['main']){print('<span>'.$resu[descr][0].'</span>');}
            $box=$this->template_include('boxes.html');
            print('<table width="100%" border="0"><tr>');
            $u=1;
            ...
            foreach($cont as $k=>$v)
            {
                ...
                foreach($v as $kk=>$vv)
                {
                    ...
                    if(!empty($vv['icon'])){$img='<img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$vv['icon'].'" height="75" align="'._ALIGN.'" style="padding:5px" />';}
                    if($j<3)
                    {
                        $item.='<li style="list-style-type: none;border:0 #ccc;border-bottom-width:1px;border-style:solid;margin-bottom:15px;height: 80px;">'.$img
                        .'<div style="font-weight: bold;"><a style="font-size:12px" title="'.clean::burn($vv[descr]).'" href="?l='.$vv['language'].'&id='.$vv['id'].'&type='.$vv['type'].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($vv['title']).'">'.clean::burn($vv[title]).'</a>'
                        .'</div><div style="color:#969696">'.$vv['_author'].'</div><div style="color:#969696">'
                        .$vv['_date']
            ...
            $u=1;
                for($i=0;$i<count($res['id']);$i++)
                {
                    if($res['permission'][$i]==1 || ($res['permission'][$i]==2 && $_SESSION['username']!='' && membership::content_member($res['id'][$i])))
                    {
                        ...
                        //if(!empty($res['_file'][$i])){if(strpos($res['_file'][$i],'.')===false){$_file=' <span style="color:#969696;">'.$res[_file][$i].'</span> ';}else{$_file='';}}else{$_file='';}
                        //if(!empty($res['icon'][$i])){$img='<img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$res['icon'][$i].'" width="90" align="'._ALIGN.'" />';}
                        print('<td dir="'._DIR.'" width="360" valign="top"><img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/icon.png" height="20" align="absmiddle" /> <a style="font-size: 14px;" href="?l='.$res['language'][$i].'&id='.$res['id'][$i].'&type='.$resu['type'][0].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'" title="'.$res['descr'][$i].'"><b>'.$res[title][$i].'</b></a></td>');
                        if($u%2==0){print('</tr><tr>');}
                        else{print('<td style="border: 0px solid;border-right-width:1px;"> <td>');}
                        $u++;
                    }
                }
                print('<tr></table>');
            ...
            break;
            case "24"://Two Title Columns with date and source
            //print('<a  href="rss.php?l='.$this->params['l'].'&id='.$resu[id][0].'"><img src="images/rssicon.png" align="'._ALIGN_.'" height="20" title="rss" alt="RSS" border="0" /></a> <span>'.$resu[descr][0].'</span>');
            print('<table width="100%" border="0"><tr>');
            $u=1;
                for($i=0;$i<count($res['id']);$i++)
                {
                    if($res['permission'][$i]==1 || ($res['permission'][$i]==2 && $_SESSION['username']!='' && membership::content_member($res['id'][$i])))
                    {
                        ...
                        //if(!empty($res['_file'][$i])){if(strpos($res['_file'][$i],'.')===false){$_file=' <span style="color:#969696;">'.$res[_file][$i].'</span> ';}else{$_file='';}}else{$_file='';}
                        //if(!empty($res['icon'][$i])){$img='<img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$res['icon'][$i].'" width="90" align="'._ALIGN.'" />';}
                        print('<td dir="'._DIR.'" width="360" valign="top"><img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/icon.png" height="20" align="absmiddle" /> <a style="font-size: 14px;" href="?l='.$res['language'][$i].'&id='.$res['id'][$i].'&type='.$resu['type'][0].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'" title="'.$res['descr'][$i].'"><b>'.$res[title][$i].'</b></a><br />'.$res['_date'][$i].' <span style="float: '._ALIGN_.';">'.$res['_author'][$i].'</span></td>');
                        if($u%2==0){print('</tr><tr>');}
                        else{print('<td style="border: 0px solid;border-right-width:1px;"> <td>');}
                        $u++;
                    }
            ...
            break;
            case "25"://عمودين من العناوين مع التاريخ والمصدر
            if(!$this->params['main']){print('<span>'.$resu[descr][0].'</span>');}
            $box=$this->template_include('boxes.html');
            print('<table width="100%" border="0"><tr>');
            $u=1;
            ...
            foreach($cont as $k=>$v)
            {
                ...
                foreach($v as $kk=>$vv)
                {
                    ...
                    $item.='<li><a title="'.clean::burn($vv[descr]).'" href="?l='.$vv['language'].'&id='.$vv['id'].'&type='.$vv['type'].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($vv['title']).'">'.clean::burn($vv[title]).'</a><br /><span style="color:#969696;float:right">'.$vv['_author'].'</span><span style="color:#969696;float:left">'.$vv['_date'].'</span><div style="clear:both"></div></li>';
                }
                $item.='</ul>';
                echo str_replace('<title_ />','<a  href="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'rss.php?l='.$this->params['l'].'&id='.$k.'"><img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'images/rssicon.png" height="20" title="rss" alt="RSS" border="0" align="'._ALIGN_.'" style="padding: 5px" /></a><b>'.$this->get_title($k).'</b>',str_replace('<tag />',$item,$box));
                print('</td>');
            ...
            break;
            case "26"://multi boxes contain titles
            if(!$this->params['main']){print('<span>'.$resu[descr][0].'</span>');}
            $box=$this->template_include('boxes.html');
            print('<table width="100%" border="0"><tr>');
            //$u=1;
            ...
            break;
            case "27"://multi boxes contain titles with date and source
            if(!$this->params['main']){print('<span>'.$resu[descr][0].'</span>');}
            $box=$this->template_include('boxes.html');
            print('<table width="100%" border="0"><tr>');
            //$u=1;
            ...
            break;
            case "33"://Two Title Columns without boxes or container
            if(!$this->params['main']){print('<span>'.$resu[descr][0].'</span>');}
            $box='<title_ /><tag />';
            print('<table width="100%" border="0"><tr>');
            $u=1;
            ...
            foreach($cont as $k=>$v)
            {
                $item='<ul style="margin:5px;padding:5px;width:100%;">';
                print('<td dir="'._DIR.'" valign="top" width="500">');
                foreach($v as $kk=>$vv)
                {
                    if($vv['type']!='' && $vv['type']!='news' && $vv['type']!='books' && $vv['type']!='vedio' && $vv['type']!='files' && $vv['type']!='pro')
                    {
                    ...
                    $item.='<li style="margin:3px;padding:3px;list-style-type:square"><a title="'.clean::burn($vv[descr]).'" href="?l='.$vv['language'].'&id='.$vv['id'].'&type='.$vv['type'].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($vv['title']).'" style="font-size:12px">'.clean::burn($vv[title]).'</a></li>';
                }
                $item.='</ul>';
                echo str_replace('<title_ />','<a  href="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'rss.php?l='.$this->params['l'].'&id='.$k.'"><img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'images/rssicon.png" height="20" title="rss" alt="RSS" border="0" align="'._ALIGN_.'" style="padding: 5px" /></a><a title="'.clean::burn($this->get_title($k)).'" href="?l='.$vv['language'].'&id='.$k.'&type='.$vv['type'].'&page='.$this->params['page'].'&event=browse&title='.clean::clean_url($this->get_title($k)).'"><b style="color:#3366FF;font-family:tahoma;">'.$this->get_title($k).'</b></a>',str_replace('<tag />',$item,$box));
                print('</td>');
                if($u%2==0){print('</tr><tr>');}
                $u++;
            }
            ...
            default:                
                ...
                for($i=0;$i<count($res['id']);$i++)
                {
                    if($res['permission'][$i]==1 || ($res['permission'][$i]==2 && $_SESSION['username']!='' && membership::content_member($res['id'][$i])))
                    {
                        ...
                        if(!empty($res['_file'][$i])){if(strpos($res['_file'][$i],'.')===false){$_file=' <span style="color:#808080;">'.$res[_file][$i].'</span> ';}else{$_file='';}}else{$_file='';}
                        block_::box_border('<table width="100%" border="0">
                        <tr style="font-size:14px;font-family:tahoma;"><td dir="'._DIR.'"><b>'.$res[title][$i].'</b></td><td dir="'._DIR.'"><span style="color:#808080;">'.$res[_author][$i].'</span>'.$_file.'</td><td dir="'._DIR.'"><span style="color:#808080;">'.$res[_date][$i].'</span></td></tr>
                        <tr><td style="font-size:14px;font-family:tahoma;vertical-align:top;" colspan="3">'.$img.'<br />'.$res['descr'][$i].' <div class="more"><a  href="?l='.$res['language'][$i].'&id='.$res['id'][$i].'&type='.$resu['type'][0].'&page='.$this->params['page'].'&event='.$event.'&title='.clean::clean_url($res['title'][$i]).'" style="color:#8E0707;font-size:11px;">'._MORE.'...</a><br /><br />'.$this->share($res,$i).'</td></tr>
    ...
    protected function browse()
    {
        ...
        $res=mysql_db::get_records_by_key("select a.id,a.title,a.icon,a.language,a.descr,a.permission,a._date,b.name as _author,c.name as _file
                                                  ...
                                                  left outer join towns c on c.id=a._file
                                                  left outer join news_type b on b.id=a._author
                                                  where a.parent='".$this->params['id']."' and a.enabled>1 $sql order by a.id desc limit ".$this->params['rec'].",$limit");
        ///switch data appearacne
        //$resu['type']=$res['type'];
        print('<span style="float: '._ALIGN_.';">');
        print('<a  href="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'rss.php?l='.$this->params['l'].'&id='.$resu[id][0].'"><img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'images/rssicon.png" align="'._ALIGN_.'" height="20" title="rss" alt="RSS" border="0" /></a>');
    ...
    protected function view()
    {
        ...
        block_::box_border('<table width="100%" border="0">
                <tr style="font-size:14px;font-family:Times New Roman,Arial, Helvetica, Sans-Serif;"><td dir="'._DIR.'"><b>'.$res[title][0].'</b></td><td dir="'._DIR.'"></td><td dir="'._DIR.'"></td><td dir="'._DIR.'"></td></tr>
                <tr><td style="font-size:11px;font-family:Times New Roman,Arial, Helvetica, Sans-Serif;vertical-align:top;" colspan="4"> &nbsp; &nbsp;<span></span>'.$img.'</td></tr>
                <tr><td style="vertical-align:top;" colspan="4"> &nbsp; &nbsp;'.embed::embed_news($res['body'][0]).' </td></tr>
                <tr><td style="vertical-align:top;" colspan="4">&nbsp;</td></tr>
                <tr><td style="font-size:11px;font-family:Times New Roman,Arial, Helvetica, Sans-Serif;vertical-align:top;" colspan="3">'.$keyword.'</td><td><a  href="" target="_blank"></a></td></tr>
                '.$share.'
                </table><br /><hr />'.$related);
        //print($res['body'][0]);
        if($res['is_control'][0]==2 && !$this->params['main'])
...
{   
    ...
    protected function view()
    {
        ...
        if(!empty($res['_file'][0]))
        {
            ...
            {
                $viewer='<iframe src="http://docs.google.com/viewer?url='.urlencode('http://'.$_SERVER['SERVER_NAME'].'/upload/'.$res['_file'][0]).'&embedded=true" width="750" height="900" style="border: none;"></iframe>';
                $dwnld_file='';
            }
            else
            {
        ...
        block_::box_border('<table width="100%" border="0">
                <tr style="font-size:14px;font-family:tahoma,Times New Roman,Arial, Helvetica, Sans-Serif;"><td dir="'._DIR.'"><b style="font-size:16px">'.$res[title][0].'</b></td><td dir="'._DIR.'">'.$res[_author][0].'</td><td dir="'._DIR.'">'.$res[_date][0].'</td></tr>
                <tr><td style="font-size:11px;font-family:Times New Roman,Arial, Helvetica, Sans-Serif;vertical-align:top;" colspan="3"> &nbsp; &nbsp;<span>'.$res['descr'][0].'</span> </td></tr>
                ...
                <tr><td style="font-size:11px;font-family:Times New Roman,Arial, Helvetica, Sans-Serif;vertical-align:top;" colspan="2">'.$keyword.'</td><td><a  href="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$res[_file][0].'" target="_blank">'.$dwnld_file.'</a></td></tr>
                '.$share.'
                </table><br /><hr />'.$related);
        //print($res['body'][0]);
        if($res['is_control'][0]==2 && !$this->params['main'])
...
{   
    protected function browse()
    {
        ...
        {            
            //print('<div id="news"><img src="upload/'.$res['icon'][$i].'" width="120" border="0" title="'.$res['title'][$i].'" align="center" />'.$res['title'][$i].'<span id="more"><a  href="?l='.$res['language'][$i].'&page='.$this->params['page'].'&type='.$this->params['type'].'&id='.$res['id'][$i].'&brand='.$this->params['brand'].'&event=view">'._MORE.'...</a></span></div><br />');
            print('<table><tr><td style="font-size:11px;" dir="'._DIR.'"></td><td>&nbsp;</td><td><a  href="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).'upload/'.$res['body'][$i].'" target="_blank"><span style="margin:20px;color:#0072bc;font-size:14px;font-family:Times New Roman,Arial, Helvetica, Sans-Serif;font-weight: bold;">'.$res['title'][$i].'</span></a></td></tr></table>');
            print('<br />');
        }
        if(!$this->params['main'])
        {            
...
{   
    protected function browse()
    {
        ...
        {            
            ...
            //block_::box_border('<table><tr><td style="font-size:11px;" dir="'._DIR.'"></td><td>&nbsp;</td><td><span style="margin:20px;color:#0072bc;font-size:14px;font-family:Times New Roman,Arial, Helvetica, Sans-Serif;font-weight: bold;">'.$res['title'][$i].'</span><br />'.$res['body'][$i].'</td></tr></table>');
            print('<br />');
        }
        if(!$this->params['main'])
        {
