...
{    function process_urls($string) 
...

                {
...

                 {
...

                    {
                        ...
                        $embed.='<td align="center"><a href="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).''.$match[1][$key].'/'.$image.'" target="_blank"><img src="http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['SCRIPT_NAME'],0,-strlen(_INDEX_PAGE)).''.$thumb.'/'.$image.'" width="100" /></a></td>';
    ...
    function view_img($string)

    {

        preg_match_all('/\[img\](.*)\[\/img\]/sU', $string, $match,PREG_PATTERN_ORDER);
...

        {
...

            {
                ...
                //{
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                    $ext=substr($match[1][$key],-4);//$ext=substr($files,-4);

            		if(($ext==".jpg" || $ext==".png" || $ext==".gif"))

                    {
                        $img=getimagesize($match[1][$key]);
                        if($img[0]>=700){$width=700;}else{$width=$img[0];}
                      $embed='<img src="'.$match[1][$key].'" width="'.$width.'" />';
                      $string=str_replace($match[0][$key],$embed,$string);
